...
def flatten(book_summaries):
    book_summaries_new = []
    for book_summ in book_summaries:
        sect_summ_new = []
        sect_summs_old = book_summ.section_summaries
        for multi_sect_title, sect_summs in sect_summs_old:
...
def manual_fix(book_summaries):
    ...
    for book_summ in book_summaries:
        ...
        while i < num_summs:
            chap_title, sect_summ = sect_summs_old[i]
            ...
            if not chap_title and empty_ss:
                i += 1
                continue
            if chap_title and not sect_summ:
                ...
                if len(arr) != 2 or not arr[1]:  # have to fix in a second pass
                    section_summaries_new.append((chap_title, sect_summ))
                    i += 1
                    continue
                ...
                if len(sect_summ1[0]) > 100:
                    ...
                    section_summaries_new.append((chap_title_orig, sect_summ_orig))
            # elif not chap
            else:
                section_summaries_new.append((chap_title_orig, sect_summ_orig))
            i += 1
...
def manual_fix_individual(book_summaries):
    ...
    for idx, book_summ in enumerate(book_summaries):
        ...
        if title in NON_NOVEL_TITLES:
            ...
            sect_summs_new = [(' '.join(chap_title.split(' ', 2)[0:2]), sect_summ)
                              for chap_title, sect_summ in sect_summs_old if chap_title]
        elif title in set(['Germinal', "Little Dorrit", "Our Mutual Friend", "The War of the Worlds"]):
            ...
            sect_summs_new = [('Chapter {}'.format(chap_title) if not chap_title.startswith('Ch') else chap_title,
                               sect_summ) for chap_title, sect_summ in sect_summs_old]
            ...
            for i, (chap_title, sect_summ) in enumerate(sect_summs_old):
                if chap_title.startswith('Book the First'):
                    ...
                    sect_summs_new.append((chap_title, sect_summ))
        elif title in set(["Gulliver's Travels", "Jude the Obscure", "Madame Bovary", 'Crime and Punishment']):  # multipart
            book_count = 0
            for i, (chap_title, sect_summ) in enumerate(sect_summs_old):
...
if __name__ == "__main__":
    ...
    book_summaries = flatten(book_summaries)
